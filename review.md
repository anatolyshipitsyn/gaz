...
| **🔐 RBAC: кол-во ролей** | **7 (кастом)** | ❌ 3 (фиксир.) | ❌ нет | ❌ нет |
| **🔐 RBAC: разграничение по цехам/площадкам** | ✅ Полное | ⚠️ Только воркспейс | ❌ нет | ❌ нет |
| **🔐 RBAC: разграничение по типу документов** | ✅ | ❌ | ❌ | ❌ |
| **🔐 RBAC: политики HSE/LOTO по роли** | ✅ | ❌ | ❌ | ❌ |
| **🔐 RBAC: SSO / LDAP / AD** | ✅ Keycloak | ⚠️ Simple passthrough | ❌ | ⚠️ Базовый |
| **🔐 RBAC: соответствие ТЗ** | ✅ **Полное** | ❌ **Не соответствует** | ❌ **Не применимо** | ❌ **Не соответствует** |
...
> **⚠️ Критический вывод по RBAC:** AnythingLLM предоставляет только 3 жёстко фиксированных роли (Admin / Manager / Default) без возможности кастомизации. Разграничение по типам документов внутри воркспейса отсутствует. Кастомные роли запрошены сообществом с 2024 года, но не реализованы. **А2 и А4 не соответствуют требованиям ТЗ по безопасности** и подходят только для демо-этапа.
...
* **Keycloak (Identity & Access):** Центральное звено безопасности. Реализует роли из ТЗ (Оператор, Механик и др.) и передает атрибуты пользователя в оркестратор для фильтрации данных.
...
* **Qdrant (Vector Database):** Хранилище векторов с поддержкой сложной фильтрации по метаданным (цех, версия документа).
...
sequenceDiagram
    ...
    UI->>KC: Проверка токена → роль + площадка + тип доступных документов
    KC-->>ORCH: ✅ Механик / Цех №2 / [инструкции, ТО] — запрет на [коммерческие]
    ORCH->>ORCH: Проверка политик (HSE/LOTO/опасные темы)
    ORCH->>RAG: Поиск с ACL-маской (цех + тип документа)
    ...
    ORCH->>DB: Лог: user / роль / запрос / ответ / doc_ids / версии
...
Аналогично с TTS: голоса Ethan / Chelsie / Aiden — это английские персонажи. Русскоязычный синтез у Qwen3-Omni значительно хуже специализированных Silero TTS или Coqui VITS, обученных на русском. Оператор в цеху будет слышать акцентированную речь.

**Мера снижения риска:** перед финальным выбором провести обязательный бенчмарк ASR на реальных записях из цеха (20–30 фраз с терминологией предприятия) и сравнить Qwen3-Omni vs. Whisper Large v3. Если ASR качество неприемлемо — использовать Whisper как отдельный сервис + Silero для TTS, отказавшись от встроенного голоса Qwen3-Omni.
...
### 🟠 RBAC в MVP: полное соответствие ТЗ за 2.5–3 месяца нереалистично

ТЗ описывает фактически **ABAC-уровень доступа**: роль × площадка × цех × тип документа. LlamaIndex не имеет встроенного механизма ACL — фильтрацию по атрибутам нужно писать вручную на уровне каждого vector store запроса. Keycloak требует настройки realm, client scopes, attribute mappers для каждой из 7 ролей.

В рамках MVP реалистичны **3–4 роли** с разграничением по воркспейсам/коллекциям Qdrant. Полные 7 ролей с атрибутным доступом по типу документа — это задача Enterprise-фазы.

**Мера снижения риска:** явно договориться с заказчиком о scope RBAC для MVP: какие роли обязательны в первой итерации, какие переносятся во вторую. Не пытаться реализовать полный ABAC в MVP — это приведёт к задержкам и техническому долгу.
...
**В4 — Аудио-среда в цехах:**
Какой уровень шума на рабочих местах, где планируется голосовой ввод? Есть ли уже записи голосовых обращений или возможность сделать тестовые записи до старта разработки? От ответа зависит: будет ли работать встроенный ASR Qwen3-Omni или нужен отдельный Whisper со шумоподавлением.

**В5 — Scope RBAC для первой итерации:**
Какие из 7 ролей ТЗ **обязательны в MVP**, а какие можно добавить во второй итерации? Какие конкретные документы или типы информации категорически нельзя показывать операторам (например, коммерческие условия, цены на ЗИП)? Без явного приоритета ролей реализация RBAC в MVP растянется и заблокирует другие задачи.
...
| **§3 Пользователи и роли** | ✅ | 7 ролей ТЗ (Оператор, Механик, Инженер, Мастер, ПТО, ИТ, Владелец знаний); доступы по площадкам/цехам и типам документов — Keycloak, ACL-фильтры |
